...
pygame.display.set_caption("Everspace!")
...
background = pygame.image.load(path.join(img_dir, 'back1.jpg')).convert()
background_rect = background.get_rect()
player_img = pygame.image.load(path.join(img_dir, "pl.png")).convert()
...
powerup_images = {}
powerup_images['shield'] = pygame.image.load(path.join(img_dir, 'remont.png')).convert()
powerup_images['shield'].set_colorkey(BLACK)
powerup_images['gun'] = pygame.image.load(path.join(img_dir, 'gold.png')).convert()
powerup_images['gun'] = pygame.transform.scale(powerup_images['gun'], (30, 30))
...
def show_go_screen():
    ...
    draw_text(screen, "Everspace!", 64, WIDTH / 2, HEIGHT / 4)
    draw_text(screen, "Стрелки для передвижения, Пробел для огня", 22,
              WIDTH / 2, HEIGHT - 250)
